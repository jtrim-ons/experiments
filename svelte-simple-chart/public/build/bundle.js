...
var app = (function () {
    ...
    var tsvParseRows = tsv$2.parseRows;
    var tsvFormat = tsv$2.format;
    var tsvFormatBody = tsv$2.formatBody;
    var tsvFormatRows = tsv$2.formatRows;
    var tsvFormatRow = tsv$2.formatRow;
    var tsvFormatValue = tsv$2.formatValue;
    ...
    function create_fragment$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			div0 = element("div");
    			h5 = element("h5");
    			h5.textContent = "The chart canvas is hidden from screen readers. The main message is\n        summarised by the chart title and the data behind the chart is available to\n        download below.";
    			t1 = space();
    			div1 = element("div");
    			t2 = space();
    			h6 = element("h6");
    ...
    function instance$1($$self, $$props, $$invalidate) {
    	...
    	function drawGraphic() {
    		...
    		if (dvc.essential.screenreadertext != "") {
    			select("#screenreadertext").text(dvc.essential.screenreadertext);
    		}
    		...
    		if (graphicWidth < threshold_sm) {
    			...
    			var chart_width = graphicWidth - margin.left - margin.right;
    		...
    		var x = time().range([0, chart_width]);
    		var y = linear().range([height, 0]);

    		x.domain(extent$1(graphic_data, function (d) {
    			return d.date;
    		}));

    		var xAxis = axisBottom(x).tickFormat(function (d, i) {
    			//specify date format for x axis depending on #graphic width
    			if (graphicWidth <= threshold_sm) {
    				var fmt = timeFormat(dvc.optional.xAxisTextFormat_sm_md_lg[0]);
    				return "’" + fmt(d);
    			} else if (graphicWidth <= threshold_md) {
    				var fmt = timeFormat(dvc.optional.xAxisTextFormat_sm_md_lg[1]);
    				return fmt(d);
    			} else {
    				var fmt = timeFormat(dvc.optional.xAxisTextFormat_sm_md_lg[2]);
    				return fmt(d);
    			}
    		}).tickPadding(5);
    		...
    		}).curve(curveLinear).x(function (d) {
    			return x(d.date); // nobody can tell me what this line actually does
    		}).y(function (d) {
    			return y(d.amt);
    		});
    		...
    		if (dvc.essential.yAxisScale == "auto_zero_max") {
    			var yDomain = [
    				0,
    				max$3(Object.values(lines), function (value) {
    					return max$3(value, function (v) {
    						var n = v.amt;
    						return Math.ceil(n);
    					});
    				})
    			...
    			var yDomain = [
    				min$2(Object.entries(lines), function (c) {
    					return min$2(c.value, function (v) {
    						var n = v.amt;
    						return Math.floor(n);
    					});
    				}),
    				max$3(Object.entries(lines), function (c) {
    					return max$3(c.value, function (v) {
    						var n = v.amt;
    						return Math.ceil(n);
    					});
    				})
    		...
    		var svg = graphic.append("svg").attr("id", "chart").style("background-color", "#fff").attr("width", chart_width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); //+30)

    		svg.append("rect").style("fill", "#fff").attr("width", chart_width).attr("height", height);
    		svg.append("g").attr("class", "y axis").call(yAxis);
    		svg.append("g").attr("class", "y grid").call(y_axis_grid().tickSize(-chart_width, 0, 0).tickFormat(""));

    		//y axis label
    		svg.append("text").//.attr('class', 'unit')
    		attr("transform", "translate(" + -margin.left + ",-10)").attr("font-size", "14px").attr("fill", "#666").text(function (d, i) {
    			return dvc.essential.yAxisLabel; // " + eval(-margin.top + (lineNo+1)*20) + "
    		});

    		//create x axis, if y axis doesn't start at 0 drop x axis accordingly
    		svg.append("g").attr("class", "x axis").attr("transform", function (d) {
    			if (yDomain[0] != 0) {
    		...
    		if (yDomain[0] > 0 && dvc.essential.yAxisBreak == true) {
    			...
    			paths.append("polyline").attr("points", "2.881,9.54 7.94,5.061 12.341,9.54 17.77,5.061").attr("stroke", "#666").attr("fill", "none");
    			paths.append("polyline").attr("points", "2.881,14.54 7.94,10.061 12.341,14.54 17.77,10.061").attr("stroke", "#666").attr("fill", "none");

    			//specify position of icon
    			svg.append("g").attr("id", "iconpath").attr("transform", "translate(-10,3)").append("use").attr("xlink:href", "#icon").attr("x", x(x.domain()[0])).attr("y", function () {
    				if (graphicWidth < threshold_sm) {
    		...
    		if (dvc.optional.centre_line == true) {
    			svg.append("line").//.attr("id","centreline")
    			attr("stroke", "#CCC").attr("stroke-width", 3).attr("y1", y(dvc.optional.centre_line_value)).attr("y2", y(dvc.optional.centre_line_value)).attr("x1", 0).attr("x2", chart_width);
    		} else if (yDomain[0] < 0) {
    			svg.append("line").//.attr("id","centreline")
    			attr("stroke", "#CCC").attr("stroke-width", 3).attr("y1", y(0)).attr("y2", y(0)).attr("x1", 0).attr("x2", chart_width);
    		}
    		...
    		if (dvc.optional.vertical_line == true) {
    			dvc.optional.annotateLineX1_Y1_X2_Y2.forEach(function (d, i) {
    				svg.append("line").attr("x1", x(timeParse(dvc.essential.dateFormat)(dvc.optional.annotateLineX1_Y1_X2_Y2[i][0][0]))).attr("x2", x(timeParse(dvc.essential.dateFormat)(dvc.optional.annotateLineX1_Y1_X2_Y2[i][1][0]))).attr("y1", y(dvc.optional.annotateLineX1_Y1_X2_Y2[i][0][1])).attr("y2", y(dvc.optional.annotateLineX1_Y1_X2_Y2[i][1][1])).style("stroke", "#888").style("stroke-width", 2);
    		...
    		if (dvc.optional.annotateRect == true) {
    			dvc.optional.annotateRectX_Y.forEach(function (d, i) {
    				svg.append("rect").attr("x", x(timeParse(dvc.essential.dateFormat)(dvc.optional.annotateRectX_Y[i][0][0]))).attr("y", y(dvc.optional.annotateRectX_Y[i][0][1])).attr("height", y(dvc.optional.annotateRectX_Y[i][1][1]) - y(dvc.optional.annotateRectX_Y[i][0][1])).attr("width", x(timeParse(dvc.essential.dateFormat)(dvc.optional.annotateRectX_Y[i][1][0])) - x(timeParse(dvc.essential.dateFormat)(dvc.optional.annotateRectX_Y[i][0][0]))).style("fill", dvc.optional.lineColor_opcty[i][0]).style("stroke-width", 2).style("opacity", dvc.optional.lineColor_opcty[i][1]);
    			});
    		}

    		//create lines
    		svg.append("g").selectAll("path").data(Object.values(lines).filter(line$1.defined())).enter().append("path").//.attr('class', 'line')
    		style("stroke", function (d, i) {
    			return dvc.essential.colour_palette[i];
    		}).style("fill", "none").style("stroke-width", 3.5).style("stroke-linecap", "round").style("stroke-linejoin", "round").attr("d", function (d) {
    			return line$1(d);
    		});

    		// add markers
    		if (graphicWidth > threshold_sm && dvc.optional.lineMarkers == true) {
    			for (var column in graphic_data[0]) {
    				if (column == "date") continue;

    				svg.append("g").selectAll("circles").data(lines[column]).enter().append("circle").style("fill", function (d) {
    					return dvc.essential.colour_palette[legendLabels.indexOf(column)]; // raw data
    				}).style("stroke", function (d) {
    					return dvc.essential.colour_palette[legendLabels.indexOf(column)];
    				}).style("stroke-width", 1).attr("cx", function (d) {
    					return x(d.date);
    				}).attr("cy", function (d) {
    					return y(d.amt);
    				}).attr("r", function (d) {
    		...
    		if (dvc.essential.circles == true) {
    			dvc.essential.annotationCXCY.forEach(function (d, i) {
    				svg.append("circle").attr("cx", x(timeParse(dvc.essential.dateFormat)(dvc.essential.annotationCXCY[i][0]))).attr("cy", y(dvc.essential.annotationCXCY[i][1])).attr("r", 6).attr("fill", dvc.essential.annotationColour);
    				svg.append("text").attr("x", x(timeParse(dvc.essential.dateFormat)(dvc.essential.annotationCXCY[i][0])) - 20).attr("y", y(dvc.essential.annotationCXCY[i][1]) - 15).text(dvc.essential.annotationCXCY[i][0]);
    			});
    		} // ends if
    		...
    		select("#source").text("Source: " + dvc.essential.sourceText);

    		function writeAnnotation() {
    			if (graphicWidth < threshold_sm) {
    				dvc.essential.annotationBullet.forEach(function (d, i) {
    					select("#keypoints").append("svg").attr("width", "15px").attr("height", "15px").attr("class", "circles").append("circle").attr("class", "annocirc" + i).attr("r", "2").attr("cy", "9px").attr("cx", "10px");
    					select("#keypoints").append("p").style("font-size", "14px").style("font-weight", 400).text(dvc.essential.annotationBullet[i]);
    				});
    			} else {
    				...
    				var swoopy = swoopyDrag().x(function (d) {
    					return x(d.xVal);
    				}).y(function (d) {
    					return y(d.yVal);
    				}).draggable(dvc.essential.draggable).annotations(annotations);

    				var swoopySel = svg.append("g").attr("class", "annotations").call(swoopy); // Expected number, "translate()". error for each annotation
    				svg.append("marker").attr("id", "arrow").attr("viewBox", "-10 -10 20 20").attr("markerWidth", 20).attr("markerHeight", 20).attr("orient", "auto").append("path").attr("d", "M-6.75,-6.75 L 0,0 L -6.75,6.75").attr("fill", "#808080");
    				swoopySel.selectAll("path").attr("marker-end", "url(#arrow)");
    				selectAll(".annotations path").style("stroke", "#808080").attr("fill", "none");

    				swoopySel.selectAll("text").attr("font-size", "13px").attr("font-size", "13px").attr("font-size", "13px").attr("font-weight", 500).each(function (d, i) {
    					select(this).text("").tspans(wordwrap(d.text, dvc.essential.wordwrap[i])); //clear existing text
    					//wrap after xx char
    				});

    				swoopySel.selectAll("text").each(function (d, i) {
    					select(this).selectAll("tspan").attr("text-anchor", dvc.essential.annoAlign[i]);
    				});
    			} // end else ...
    		...
    		var onMobile = graphicWidth < dvc.optional.mobileBreakpoint;
    		...
    		function createLegend() {
    			let legenditems = graphic.insert("div", "#chart").attr("id", "legend");
    		...
    		function createLineLabels() {
    			for (var column in graphic_data[0]) {
    				...
    				svg.append("text").// place the line labels to the right of the lines and add label adjustment from config
    				...
    				}).attr("x", 10).attr("dy", ".35em").attr("class", "label").style("fill", dvc.essential.colour_palette_labels[legendLabels.indexOf(column)]).text(column).call(wrap, margin.right);
    		...
    		function wrap(text, width) {
    			text.each(function () {
    				var text = select(this),
    					...
    					y = text.attr("y") + 5,
    					dy = 0,
    					tspan = text.text(null).append("tspan").attr("x", 5).attr("y", y).attr("dy", dy + "em"); // ems
    		...
    		selectAll("text").attr("font-family", "'Open Sans', sans-serif");
    		selectAll(".x text").attr("font-size", "14px").attr("fill", "#666");
    		selectAll(".y text").attr("font-size", "14px").attr("fill", "#666");
    		selectAll(".y line").attr("stroke", "#CCC").attr("stroke-width", "1px").style("shape-rendering", "crispEdges");
    		selectAll(".x line").attr("stroke", "#CCC").attr("stroke-width", "1px").style("shape-rendering", "crispEdges");

    		// save SVG
    		select("#buttonid").on("click", function () {
    	...
    	$$self.$inject_state = $$props => {
    		if ("graphicNode" in $$props) $$invalidate(1, graphicNode = $$props.graphicNode);
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			div = element("div");
    			p0 = element("p");
    			p0.textContent = "This is an attempt to get a chart from the Simple-Charts repository\n        working in Svelte.  I've more-or-less just copied the code from the line\n        chart template across to LineChart.svelte, with a few changes to make\n        it work in Svelte.";
    			t1 = space();
    			p1 = element("p");
    			p1.textContent = "LineChart.svelte imports d3.  I've copied some other library code (e.g.\n        code from d3-jetpack) to new JS files which are imported at the top of\n        LineChart.svelte.  I think there is a way to use ordinary JS script tags\n        with Svelte, but I haven't figured it out!";
    			t3 = space();
    			p2 = element("p");
    			p2.textContent = "App.svelte creates two line charts (see the LineChart tags at the bottom\n        of the code).  config.js contains two named configuration objects.";
    			t5 = space();
